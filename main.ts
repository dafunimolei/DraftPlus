...
class DraftPlusModal extends Modal {
	...
	safe(sa: string) {
		...
		const removePrefix = (str: string, prefixes: Array<string>) => {
			const prefixPattern = new RegExp(`^(${prefixes.join('|')})`);
			return str.replace(prefixPattern, '');
		};
		const containsLetters = (str: string) => {
			const letterPattern = /[a-zA-Z]/;
			return letterPattern.test(str);
		};
		const prefixes = ["http:", "httpx:", 'ws:', 'www.', "http:/", "httpx:/", 'ws:/', "http://", "httpx://", 'ws://', ];
		let sad = removeSpaces(sa);
		sad = removePrefix(sad, prefixes);
		
		if (containsLetters(sad) == true) {
			new Notice('服务器地址中含有字母，应遵从ip号：端口号的形式！');
		}
		return sad;
	}
	setupWebSocket1() {
...
class DraftPlusSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('服务器地址，为ip号：端口号的形式。')
			.addText(text => text
				.setPlaceholder('格式为：xx.xx.xx.xx:xxx')
